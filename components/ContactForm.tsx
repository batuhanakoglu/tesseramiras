...
export const ContactForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. Yerel Kayıt (Sadece gönderen kişinin tarayıcısında kalır)
    addMessage({
    ...
    // 2. Mailto Protokolü (Sahibine ulaşması için gerçek kanal)
      ...
      {submitted ? (
        <div className="border border-accent p-12 text-center space-y-6 bg-accent/5 animate-entry">
          ...
          <p className="text-[10px] opacity-40 uppercase max-w-xs mx-auto leading-relaxed">
            Lütfen açılan e-posta uygulamanızdan "Gönder" butonuna basarak iletimi tamamlayın.
          </p>
        </div>
      ) : (
