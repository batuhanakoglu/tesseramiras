...
export const Admin: React.FC = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'POST' | 'ANN' | 'AUTHOR') => {
    ...
    try {
      ...
      alert("Görsel başarıyla GitHub'a yüklendi.");
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-black flex flex-col lg:flex-row text-white font-sans selection:bg-accent">
      <aside className="w-full lg:w-64 border-r border-white/5 p-8 flex flex-col bg-[#050505] lg:h-screen lg:sticky lg:top-0">
        ...
        <div className="space-y-3 mb-8">
          ...
          <button onClick={() => refreshFromGitHub()} className="w-full py-2 border border-white/10 text-[8px] font-bold uppercase tracking-widest hover:bg-white/5 transition-colors">PULL_FROM_CLOUD</button>
        </div>
        ...
        {activeTab === AdminTab.POSTS && (
          <div className="space-y-12 animate-entry">
            <form onSubmit={e => { 
              ...
              <h3 className="text-[10px] font-black uppercase text-accent tracking-widest italic">[{editingId ? 'MODE: EDIT_ENTRY' : 'MODE: NEW_ARCHIVE_LOG'}]</h3>
              <input type="text" placeholder="BAŞLIK" required className="w-full bg-transparent border-b border-white/10 py-4 text-2xl font-black uppercase focus:outline-none focus:border-accent text-white" value={postForm.title} onChange={e => setPostForm({...postForm, title: e.target.value})} />
              ...
              <textarea placeholder="ÖZET METNİ" className="w-full bg-transparent border-b border-white/10 py-2 text-sm italic text-white" value={postForm.excerpt} onChange={e => setPostForm({...postForm, excerpt: e.target.value})} />
        ...
        {activeTab === AdminTab.ANNOUNCEMENTS && (
          <div className="space-y-12 animate-entry">
            <form onSubmit={e => { 
              ...
              <h3 className="text-[10px] font-black uppercase text-accent tracking-widest italic">[{editingAnnId ? 'MODE: EDIT_NEWS' : 'MODE: NEW_ANNOUNCEMENT'}]</h3>
              ...
              <div className="flex gap-4 items-center">
                ...
                <input type="file" className="hidden" id="ann-image-upload" onChange={e => handleImageUpload(e, 'ANN')} accept="image/*" />
                <button type="button" onClick={() => document.getElementById('ann-image-upload')?.click()} className="px-4 py-2 border border-white/20 text-[9px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all">
                  {isUploading ? 'UPLOADING...' : 'DOSYA_SEÇ'}
                </button>
              </div>
              <textarea placeholder="DUYURU İÇERİĞİ" className="w-full bg-transparent border-b border-white/10 py-2 text-sm italic outline-none text-white" value={annForm.content} onChange={e => setAnnForm({...annForm, content: e.target.value})} />
              ...
              <button type="submit" className="w-full bg-accent py-5 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all">DUYURUYU_GÜNCELLE</button>
            </form>
            <div className="space-y-4">
        ...
        {activeTab === AdminTab.MESSAGES && (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 animate-entry">
             <div className="md:col-span-5 space-y-2 max-h-[60vh] overflow-y-auto custom-scrollbar">
                ...
                )) : <div className="p-12 text-center opacity-20 uppercase text-[9px] tracking-widest border border-dashed border-white/5">GELEN KUTUSU BOŞ</div>}
             </div>
             <div className="md:col-span-7 bg-white/5 border border-white/5 p-8 flex flex-col min-h-[400px]">
                ...
                ) : <div className="m-auto opacity-10 uppercase text-[9px] tracking-[0.5em] italic">İLETİ SEÇİNİZ</div>}
             </div>
          </div>
        )}
